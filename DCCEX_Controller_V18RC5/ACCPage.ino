...
//        accSlot;
        ...
        switch (eMode)
        {
          ...
          {
            if(hcAcc[accStartID + accSlot].AccAddress != 0)
            {
          ...
          case (2):       //Select Mode Active
          {
            ...
            if(button >= SlotBStart && button < (SlotBStart + accessoriesPerPage))     //These are the Accessory Images buttons ONLY processed in Select Mode
            {
              ...
              { 
                ...
                if(accStates[accStartID + accSlot] == false)
                {
                  nextionString = iSlot + ".pic=" + String(hcAcc[accStartID + accSlot].AccImage);      //readAccImage(accStartID + accSlot));      // + accStates[accStartID + accSlot]));
                }else{
                  nextionString = iSlot + ".pic=" + String(hcAcc[accStartID + accSlot].AccImage + 1);              
                }
                nextionCommand(nextionString.c_str());
