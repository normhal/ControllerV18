...
void stopButton()
{
  if (readEEPROMByte(eeLocoStopAll) == 1)
  {
    ...
    {
      resumeSpeeds[i] = hcLoco[selectedIDs[thNum][i]].speed;
      hcLoco[selectedIDs[thNum][i]].speed = 0;
      doDCC(i);
    }
    activateSlot(activeSlot[thNum]);
  } else {
    resumeSpeeds[activeSlot[thNum]] = hcLoco[selectedIDs[thNum][activeSlot[thNum]]].speed;
    hcLoco[selectedIDs[thNum][activeSlot[thNum]]].speed = 0;
...
void goButton()
{
  if (readEEPROMByte(eeLocoStopAll) == 1)
  {
    ...
    for (uint8_t i = 0; i < numLocoSlots; i++)
    {
      hcLoco[selectedIDs[thNum][i]].speed = resumeSpeeds[i];
      doDCC(i);
    }
    activateSlot(activeSlot[thNum]);
  } else {
    hcLoco[selectedIDs[thNum][activeSlot[thNum]]].speed = resumeSpeeds[activeSlot[thNum]];
    doDCC(activeSlot[thNum]);
    activateSlot(activeSlot[thNum]);
  }
}
