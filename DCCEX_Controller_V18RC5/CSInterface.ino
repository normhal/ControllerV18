...
uint8_t connectWiFi(uint8_t retries)
{
    ...
    WiFi.begin(cred[credIndex].SSID.c_str(), cred[credIndex].Password.c_str());
    int i = 0;
    wifiProgress = ("SSID Selected: " + cred[credIndex].SSID);
    ...
    Console.printf("Using SSID: %s\n", cred[credIndex].SSID);
    Console.printf("and Password: %s\n", cred[credIndex].Password);
    Console.printf("and IP Address: %s\n", cred[credIndex].IPAddress);
    Console.printf("on Port: %d\n", cred[credIndex].Port);
    ...
    if(client.connect(cred[credIndex].IPAddress.c_str(), cred[credIndex].Port))              //DCCEX Host on port 2560
    {
...
{
  String dccppCMD = "";
  if(hcLoco[selectedIDs[thNum][locoSlot]].LocoAddress != 0){
    #if defined DCCPP
      dccppCMD = ("<t " + String(selectedIDs[thNum][locoSlot] + 1) + " " + String(hcLoco[selectedIDs[thNum][locoSlot]].Address)) + " "
             + String(hcLoco[selectedIDs[thNum][locoSlot]].speed ) + " " + String(hcLoco[selectedIDs[thNum][locoSlot]].direction ) + ">");
    #else
      dccppCMD = ("<t " + String(hcLoco[selectedIDs[thNum][locoSlot]].LocoAddress) + " "
             + String(hcLoco[selectedIDs[thNum][locoSlot]].speed ) + " " + String(hcLoco[selectedIDs[thNum][locoSlot]].direction ) + ">");
    #endif
    sendCMD(dccppCMD);
  }
...
{
  String dccppCMD = "";
  dccppCMD = ("<f " + String(hcLoco[selectedIDs[thNum][activeSlot[thNum]].LocoAddress) + " " + String(fGroup5)+
...
//Required command is "<F Address Function State>"
  ...
  {
    String dccppCMD = "";
    dccppCMD = ("<F " + String(hcLoco[selectedIDs[thNum][activeSlot[thNum]]].LocoAddress) +
              " " + String(fNum) + 
              " " + String(hcLoco[selectedIDs[thNum][activeSlot[thNum]]].FSlot[g_fSlot][2]) + ">");
    sendCMD(dccppCMD);
  }
#endif
/*
...
{
  String dccppCMD = "";
  dccppCMD = ("<a " + String(hcAcc[accID].AccAddress) + " " + String(accStates[accID]) + ">");
...
void updateWiFi()
{
    ...
    if(WiFiEnabled == 1)
    {
      ...
      nextionSetText("RSSI", String(WiFi.RSSI()));
      wait(200);
      if(!client.connect(cred[credIndex].IPAddress.c_str(), cred[credIndex].Port))
      {
