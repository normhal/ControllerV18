...
Initial Uploading/flashing for Wemos D1 Mini (ESP8266 with 18650 battery):

1. select board "LOLIN (WEMOS° D1 R2 & mini") (boards ESP8266 v3.1.2)
2. remove Nextion TX and RX connection
3. battery must be in holder
4. switch ON
5. press RST, keep pressed, press Flash, release RST, then release Flash (after upload is started)
...
Hardware Serial RX - ESP32 pin GPIO16 to Nextion TX wire (Blue) (Wemos: D16)
Hardware Serial TX - ESP32 pin GPIO17 to Nextion RX wire (Yellow) (Wemos: D17)
...
4. switch ON
5. press RST (or EN), keep pressed, press FLASH (or BOOT), release RST (EN). Keep FLASH (or BOOT) pressed until flash has actually started
6. compile and flash

Note: After the initial programming, OTA can be used without requiring any intervention.
https://forum.arduino.cc/t/raspberry-pi-pico-cant-load-any-sketch/1008640/9


Pin Assignments for RaspBerry Pi Pico and Pico W

IMPORTANT!! First time use of a Pico requires pressing and holding the BOOTSEL button before USB connection !!
Solution: https://forum.arduino.cc/t/raspberry-pi-pico-cant-load-any-sketch/1008640/9
https://forum.arduino.cc/t/upload-to-rpi-pico-w-issue/1319260/5
The issue of "hard hang" upon flash start on Pico is described on https://github.com/earlephilhower/arduino-pico
But when the BOOTSEL button procedure has to be repeated EVERY time then a hard reset is required:
1. flash the Pico with flash_nuke.uf2 (from this official Pico website)
2. reset the USB connections stack on Pico in Device Manager
3. start with the first flash using BOOTSEL button procedure
4. next flashes without BOOTSEL procedure
...
Nextion Display to 2nd Serial port - UART1
...
// Set unused Loco IDs to unused (255)
  for(int i = 0; i < tCount; i++)
  {
    for(int j = 0; j < numLocoSlots; j++)
    {
      selectedIDs[i][j] = 255;
    }
  }
// Set unused Route IDs to unused (255)
  for(int i = 0; i < numRoutes; i++) hcRoute[i].AccIDs[0][0] = 255;
...
/*
  listDir(LittleFS, "/", 0);
  Serial.println("LittleFS Setup and Successfully Read");
*/
  Serial.println("Now populating Locos...");
  populateLocoArray("/locos.txt");
  Serial.println("Now adding Functions to Locos");
  populateLocoFunctions("/functions.txt");
  Serial.println("Now populating Accessories...");
  populateAccArray("/accessories.txt");
  Serial.println("Now populating Routes...");
  populateRoutesArray("/routes.txt");
  Serial.println("Now loading List of Credentials");
  populateCredArray("/credentials.txt");
  Serial.println("Now loading Selected Loco IDs");
  populateSelected("/throttleids.txt");
  Serial.println("All Populated");
//
//*****************************************************************************************************
// Initialize Nextion and display Version Number
//*****************************************************************************************************
//
  ...
  for(uint8_t r = 0; r <tCount; r++) activeSlot[r] = 0;
