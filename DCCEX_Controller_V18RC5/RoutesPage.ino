  ...
  switch(button)
  {
    ...
    case AccButton:
      ...
      { 
        ...
        {
          uint16_t accID = hcRoute[selectedRoute].AccIDs[r][0];
          uint16_t accAddr = hcAcc[accID].AccAddress;               //readAccAddress(accID);
          uint16_t accState = hcRoute[selectedRoute].AccIDs[r][1];
          if(accID != 0)
          {
            String CMD = "<a " + String(accAddr) + " " + String(accState) + ">";
            sendCMD(CMD);
...
void routeDrawPage(uint8_t routeStartID)
{
  for(uint8_t r = 0; r < rowsPerNextionRoutePage; r++)       //Prepare to draw "rowsPerNextionRoutePage" rows
  {
    ...
    {
      if(hcRoute[routeStartID + r].AccIDs[d][0] != 0)           //; readEEPROMByte(eeRouteAddress + d) != 255)
      {
        uint8_t accID = hcRoute[routeStartID + r].AccIDs[d][0];       //Byte(eeRouteAddress + d);
        uint16_t accAddr = hcAcc[accID].AccAddress;     //       readAccAddress(accID);
        uint16_t accState = hcRoute[routeStartID + r].AccIDs[d][1];
        if(entryNum<3) routeDetail1 = (routeDetail1 + String(accAddr) + ":" + String(accState) + "   ");
        else routeDetail2 = (routeDetail2 + String(accAddr) + ":" + String(accState) + "   ");
        entryNum++;
      }
    }
