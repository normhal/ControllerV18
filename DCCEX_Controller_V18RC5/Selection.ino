    ...
    default:
    {
      if(button >= 240 && button <= 249)
      {
        ...
        switch(listName)
        {
          case SSIDs:
            credIndex = button;
            cred[credIndex].SSID = foundSSID[credIndex];
            break;
          case uSSIDs:
      ...
      case CREDs:
        for(int i=0; i<10; i++)
        {
          if(cred[i].SSID != "")
          {
            ...
            nextionSetText("b"+String(i+10), String(cred[i].IPAddress));
          }
        }
...
/*
      case TO_SAVE:
        if(accsDirty) saveAccs;
        if(credsDirty) saveCreds;
        if(funcsDirty) saveFuncs;
        if(locosDirty) saveLocos;
        if(routesDirty) saveRoutes;
        if(throtIdsDirty) saveIDs;
        break;
      case TO_RESTORE:
        break;
*/
