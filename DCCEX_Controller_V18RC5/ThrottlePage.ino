    ...
    default:                                                  
    {
      ...
      if(selectedIDs[thNum][activeSlot[thNum]] != 255)        
      {
        if(button >= FunctionSlotStart && button < (FunctionSlotStart + functionsPerPage))         //Process the Function Slots
        {
          ...
          if(hcLoco[selectedIDs[thNum][activeSlot[thNum]]].FSlot[g_fSlot][1] == 0) break;       //LocoID = 0
          uint8_t funcNum = hcLoco[selectedIDs[thNum][activeSlot[thNum]]].FSlot[g_fSlot][0];    //retrieve the actual function number from its Loco record
          if(funcNum == 127) break;
          uint8_t funcImg = hcLoco[selectedIDs[thNum][activeSlot[thNum]]].FSlot[g_fSlot][1];
        ...
        {
          if(hcLoco[selectedIDs[thNum][activeSlot[thNum]]].FSlot[g_fSlot][1] == 0) break;
          uint8_t funcNum = hcLoco[selectedIDs[thNum][activeSlot[thNum]]].FSlot[g_fSlot][0];   //retrieve the actual function number from its EEPROM slot
  ...
  for(uint8_t i = 0; i < numLocoSlots ; i++)
  {
    if(i != activeSlot[thNum])
    {
      if(selectedIDs[thNum][i] != 255)
      {
        ...
        nextionSetText("t" + String(i), hcLoco[selectedIDs[thNum][i]].LocoType);                      //Loco Type
        nextionSetText("n" + String(i), hcLoco[selectedIDs[thNum][i]].LocoShortName);                   //Road Name
        if(hcLoco[selectedIDs[thNum][i]].LocoAddress == 0)
        {
        ...
        }else
        {
          ...
          {
            nextionSetText("v" + String(i), String(hcLoco[selectedIDs[thNum][i]].LocoAddress));
          }else
          {
            nextionSetText("v" + String(i), String(hcLoco[selectedIDs[thNum][i]].LocoRNum));
          }
...
void activateSlot(uint8_t slot)
{
  if(nextionPage == ThrottlePage)
  {
      ...
      nextionCommand("v" + String(slot) + ".bco=" + String(GREY));   //Address/Road Number Field background Colour
    #endif
    if(hcLoco[selectedIDs[thNum][slot]].LocoAddress == 0 || selectedIDs[thNum][slot] == 255)
    {
    ...
    { 
      ...
      {
        nextionSetText("LName", hcLoco[selectedIDs[thNum][slot]].LocoLongName);
        nextionSetText("n" + String(slot), hcLoco[selectedIDs[thNum][slot]].LocoShortName);         //Road Name
        nextionSetText("t" + String(slot), hcLoco[selectedIDs[thNum][slot]].LocoType);       //Loco Type  
        if(readEEPROMByte(eeRNumEnabled) == 0)
        {
          nextionSetText("v" + String(slot), String(hcLoco[selectedIDs[thNum][slot]].LocoAddress));
        }else{
          nextionSetText("v" + String(slot), String(hcLoco[selectedIDs[thNum][slot]].LocoRNum));
        }
        nextionCommand("v" + String(slot) + ".pco=" + String(BLACK));   //Font Colour
        updateNextionThrottle(hcLoco[selectedIDs[thNum][slot]].speed);
        if(hcLoco[selectedIDs[thNum][slot]].direction == 1) nextionCommand("FR.pic=8");
        else nextionCommand("FR.pic=9");
        nextionSetText("AD", String(hcLoco[selectedIDs[thNum][slot]].LocoAddress));
...
void loadFunctions(uint8_t Page, uint8_t locoID)
{
  ...
  {
    uint8_t fNum = hcLoco[locoID].FSlot[l_fSlot][0];  //Function Number
    fNum = fNum & 0x7F;                               //Remove the pulse indicator bit
    uint8_t iNum = hcLoco[locoID].FSlot[l_fSlot][1];
    if(iNum == 0) fNum = 127;
    if (fNum <= 68)        //Maximum function number supported at this time
    {
      ...
      String nextionString = ("s" + String(l_fSlot) + ".pic=" + String(iNum + hcLoco[locoID].FSlot[l_fSlot][2])); //Function Image Slot
...
//      functions[selectedIDs[thNum][activeSlot[thNum]]][g_fSlot]=1;
      hcLoco[selectedIDs[thNum][activeSlot[thNum]]].FSlot[g_fSlot][2] = 1;
    }
    else
    {
      bitWrite(LocoFN0to4[selectedIDs[thNum][activeSlot[thNum]]], 4, 0);
//      functions[selectedIDs[thNum][activeSlot[thNum]]][g_fSlot]=0;
      hcLoco[selectedIDs[thNum][activeSlot[thNum]]].FSlot[g_fSlot][2] = 0;
    }
    doDCCfunction04();
    }
...
//        functions[selectedIDs[thNum][activeSlot[thNum]]][g_fSlot]=0;
  ...
  #if !defined DCCPP
    if(hcLoco[selectedIDs[thNum][activeSlot[thNum]]].FSlot[g_fSlot][2] == 0)
    {
      funcImg = funcImg + 1;
      hcLoco[selectedIDs[thNum][activeSlot[thNum]]].FSlot[g_fSlot][2] = 1;
    }
...
void changeDirection()
{
  ...
  { 
    ...
    {
      hcLoco[selectedIDs[thNum][currentSlot]].speed = 0;
      encoderPos = 0;
      oldEncPos = 0;
    }
    if(hcLoco[selectedIDs[thNum][currentSlot]].direction == 1)       //Currently Forward
    {
      hcLoco[selectedIDs[thNum][currentSlot]].direction = 0;         //Set to Reverse
      nextionCommand("FR.pic=9");
    }else
    {                                                         //Currently Reverse
      hcLoco[selectedIDs[thNum][currentSlot]].direction = 1;         //Set to Forward
