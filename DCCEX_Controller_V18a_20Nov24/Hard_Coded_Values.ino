...
void loadHardCodedValues()
{
  ...
  /************************************************************************************************************
  * Writes Hard Coded details of Locos and Functions to EEPROM
  /************************************************************************************************************/
  thNum = 0;                                    //Reset Throttle Number to 0
  /*
  for(uint8_t i = 0; i < NUM_HCLOCOS; i++)
  { 
    writeEEPROMName((locoNameBase + (i * (locoNameLen))), ((hcLoco[i].LocoShortName) + '\0'));
    writeEEPROMName((locoTypeBase + (i * (locoTypeLen))), ((hcLoco[i].LocoType) + '\0'));
    writeLocoAddress(i, hcLoco[i].LocoAddress);
    writeLocoRNum(i, hcLoco[i].LocoRNum);
    ...
    for(uint8_t j = 0; j < numFSlots; j++)      //Number of configured function Slots (10 default)
    {
      if(hcLoco[i].FSlot[j][2] != 0)              //only write details if Function Image is present
      {
        writeEEPROMByte((locoFuncBase + (i * 20)) + (hcLoco[i].FSlot[j][0]*2), (hcLoco[i].FSlot[j][1]) | (hcLoco[i].FSlot[j][3]));    //Write the Function number into the correct Slot and address

        writeEEPROMByte((locoFuncBase + (i * 20)) + (hcLoco[i].FSlot[j][0]*2) + 1, hcLoco[i].FSlot[j][2]);  //Write the Image Number into the adjacent Slot and address
      }
    }
  }
  */
