...
{  
  for(uint8_t i = 0; i <= numLocoSlots ; i++)
  {
    ...
    {
      Serial.printf("LocoID read: %d for Slot: %d\n", selectedIDs[thNum][i], i);
      if(selectedIDs[thNum][i] != 255)
      {
        ...
        Serial.printf("Throttle: %d Slot: %d contains Loco ID: %d\n", thNum, i, selectedIDs[thNum][i]);
...
//    uint8_t fNum = readEEPROMByte((locoFuncBase + (locoID * 20)) + (l_fSlot*2));         // 20 bytes needed for 10 slots on a page
    uint8_t fNum = hcLoco[locoID].FSlot[l_fSlot][0]; //[adEEPROMByte((locoFuncBase + (locoID * 20)) + (l_fSlot*2));         // 20 bytes needed for 10 slots on a page
