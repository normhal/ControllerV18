...
//        accSlot;
        ...
        switch (eMode)
        {
          ...
          {
            if(hcAcc[accStartID + accSlot].AccImage != BLANK)
            {
              ...
              {
                nextionString = iSlot + ".pic=" + String(hcAcc[accStartID + accSlot].AccImage);      // + accStates[accStartID + accSlot]));
              }else{
                nextionString = iSlot + ".pic=" + String(hcAcc[accStartID + accSlot].AccImage + 1);              
              }
...
void accDrawPage(uint8_t accStartID) 
{
  ...
  for (uint8_t accID = accStartID; accID < accStartID + accessoriesPerPage; accID++)
  {
    ...
    Serial.printf("AccID: %d, Name: %s, Address: %d, Image: %d, Type: %d\n", hcAcc[accID].AccId,hcAcc[accID].AccName,hcAcc[accID].AccAddress,hcAcc[accID].AccImage,hcAcc[accID].AccType);
    if (hcAcc[accID].AccAddress != 0)                                                                    //Read from local EEPROM >0 means occupied
    { 
      nextionSetText(pSlot, hcAcc[accID].AccName);
      nextionSetText(aSlot, String(hcAcc[accID].AccAddress));                             //Set the Accessory Address
      nextionCommand(iSlot + ".pic=" + String((hcAcc[accID].AccImage) + accStates[accID]).c_str());               //Set the Accessory Image with State
    }else
    {
